...
import { isMetal, OZ_TO_GRAM } from "@/lib/metals"
...
export function HomeClient({ initialGoldUsd, initialSilverUsd }: HomeClientProps) {
  const [base, setBase] = useState("USD")
  const [target, setTarget] = useState("EUR")
  const { data } = useExchangeRate(base, target)
  const currentRate = data?.rates[target] ?? null

  // Metal prices for Precious Metals section (live updates, seeded with build-time values)
  const { price: goldUsd } = useMetalRate("XAU", initialGoldUsd)
  const { price: silverUsd } = useMetalRate("XAG", initialSilverUsd)

  const goldGram = goldUsd !== null ? goldUsd / OZ_TO_GRAM : null
  const silverGram = silverUsd !== null ? silverUsd / OZ_TO_GRAM : null
      ...
      <div className="relative z-10 min-h-screen flex flex-col">
        ...
        <main className="flex-1 container mx-auto px-4 py-6 max-w-6xl">
          <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-4">
                <div>
                  ...
                  <p className="mt-2 text-slate-400 text-sm">
                    Real-time exchange rates for 15 major currencies and precious metals
                  </p>
                </div>
                <CurrencyConverter
                  base={base}
            ...
            {/* Historical table — hidden when a metal is selected */}
            ...
            {/* Precious Metals */}
            <section className="flex flex-col gap-4 animate-fade-in-up" style={{ animationDelay: "150ms" }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Link
                  href="/gold-vs-usd"
                  className="glass-card p-5 flex flex-col gap-3 hover:border-[#6366f1]/40 hover:scale-[1.01] transition-all group cursor-pointer"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-2xl">🥇</span>
                      <span className="text-base font-bold text-white font-heading group-hover:text-[#6366f1] transition-colors">
                        Gold (XAU)
                      </span>
                    </div>
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex items-baseline gap-2">
                      <span className="text-2xl font-bold font-mono text-white">
                        {goldUsd !== null ? `$${fmt(goldUsd, 2)}` : "—"}
                      </span>
                      <span className="text-xs text-slate-500 font-mono">/ oz</span>
                    </div>
                    <div className="text-sm font-mono text-slate-400">
                      {goldGram !== null ? `$${fmt(goldGram, 2)} / g` : ""}
                    </div>
                  </div>
                  <div className="text-[10px] text-[#6366f1] font-mono uppercase tracking-wider group-hover:underline">
                    See all gold prices →
                  </div>
                </Link>
                ...
                >
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex items-baseline gap-2">
                      <span className="text-2xl font-bold font-mono text-white">
                        {silverUsd !== null ? `$${fmt(silverUsd, 2)}` : "—"}
                      </span>
                      <span className="text-xs text-slate-500 font-mono">/ oz</span>
                    </div>
                    <div className="text-sm font-mono text-slate-400">
                      {silverGram !== null ? `$${fmt(silverGram, 2)} / g` : ""}
                    </div>
                  </div>
                  <div className="text-[10px] text-[#6366f1] font-mono uppercase tracking-wider group-hover:underline">
                    See all silver prices →
                  </div>
                </Link>
              </div>
            </section>
            ...
            <section className="flex flex-col gap-6 animate-fade-in-up" style={{ animationDelay: "300ms" }}>
              <div className="glass-card p-6 md:p-8 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3 text-sm text-slate-300 leading-relaxed font-body">
                  <p>
                    CurrencyVersus is a free tool for comparing exchange rates between 15 major world currencies — including USD, EUR, GBP, JPY, TRY, CNY, INR, and more — plus live gold and silver prices.
                  </p>
                  <p>
                  ...
                  <p>
                    Whether you{"'"}re a traveler planning a trip, an investor monitoring forex and precious metals markets, or a student learning about global economics, CurrencyVersus makes it easy to understand financial trends without any sign-up or fees.
                  </p>
                </div>
              </div>
                  ...
                  {[
                    {
                      ...
                      desc: "Choose any two of 15 major world currencies or precious metals (Gold, Silver) to compare — from US Dollar to Turkish Lira and beyond.",
                    },
                    {
                      step: "02",
        ...
        <footer className="text-center py-6 flex flex-col gap-1">
          <p className="text-xs text-slate-500 font-mono">
            FX data from European Central Bank via{" "}
            <a
